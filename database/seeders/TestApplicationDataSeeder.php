...
class TestApplicationDataSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $alldata = Items::fromFile(storage_path('app/301-350kkk.json'));

        $banks = Bank::all()->keyBy('name');
        $cities = City::all()->keyBy('name');
        $branches = Branch::with('city')->get()->keyBy('name');
        $vehicles = Vehicle::all()->keyBy('name');
        $sources = Source::all()->keyBy('name');
        ...
        foreach($alldata as $singleData){
                ...
                foreach ($optionalData as $key => $value) {
                    if ($value) {
                        switch ($key) {
                            case 'bank':
                                $bank = $banks[$value] ?? Bank::firstOrCreate(['name' => $value]);
                                $bank_id = $bank->id;
                                break;
                            case 'dealer_city':
                                $city = $cities[$value] ?? City::firstOrCreate(['name' => $value]);
                                $city_id = $city->id;
                                break;
                            case 'dealer_branch':
                                if ($city) {
                                    $branch = $branches[$value] ?? Branch::firstOrCreate(['name' => $value, 'city_id' => $city->id]);
                                    $branch_id = $branch->id;
                                }
                                break;
                            case 'vehicle':
                                $vehicle = $vehicles[$value] ?? Vehicle::firstOrCreate(['name' => $value]);
                                $vehicle_id = $vehicle->id;
                                break;
                            case 'channel':
                                $source = $sources[$value] ?? Source::firstOrCreate(['name' => $value]);
                                $source_id = $source->id;
                                break;
                            case 'campaign':
                                $campaign = $campaigns[$value] ?? Campaign::firstOrCreate(['name' => $value]);
